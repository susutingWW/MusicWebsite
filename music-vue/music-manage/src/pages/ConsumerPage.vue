  ...
  <div class="table" style="height: 100%;width: 100%">
    <div class="container" style="height: 8%; display: flex;align-items: center;justify-content: center;">
      <div class="handle-box">
        ...
        <el-button type="primary" size="mini" @click="centerDialogVisible=true" style="margin-left: 150px">添加新用户</el-button>
      </div>
    </div>
    <el-table size="mini" border style="width:100%" height="84%" :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="phoneNum" label="手机号" width="100" align="center"></el-table-column>
      <el-table-column prop="email" label="电子邮箱" width="100" align="center"></el-table-column>
      <el-table-column label="生日" width="100" align="center" prop="birth">
        <template slot-scope="scope">
...
<script>
import {delConsumer, getAllConsumer, setConsumer, updateConsumer} from "@/api";
...
export default {
  ...
  methods:{
    addressChoose(value) {
      ...
      this.form.location = value[0] + "/" + value[1] + "/" + value[2];
    },
    addressChoose1(value) {
      console.log("##选中的地区", value);
      this.registerForm.location = value[0] + "/" + value[1] + "/" + value[2];
    },
    //添加歌手
    addConsumer(){
      this.$refs['registerForm'].validate(valid =>{
        if(valid){
          ...
          params.append('avator','avatorImages/consumer.jpg');
          console.log(params);
          setConsumer(params)
              .then(res=>{
                console.log(res);
